...
# Creates a temp copy of the provided directory.
# Ignore unwanted files like __init__.py, test files.
# Returns the temporary path and a list of only files, no directories.
...
# Creates a GCS client and connects to the target bucket.
#Iterates over each file in the temp directory and uploads it to:  #gs://<bucket_name>/<name_replacement>/<file_path>
#name_replacement determines if the file goes into dags/ or data/.
...
# This block parses command-line arguments and then:
# Uploads DAGs — if --dags_directory exists.
# Uploads data files — if --data_directory exists.
